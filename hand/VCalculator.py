...
class Button:
    ...
    def text(self, value, img):
        self.value = value
        cv2.putText(img, self.value, (self.pos[0] + 25, self.pos[1] + 50), cv2.FONT_HERSHEY_PLAIN, 3, (255, 255, 255), 3)
        
    def text_hover(self, value, img):
        self.value = value
        cv2.putText(img, self.value, (self.pos[0] + 25, self.pos[1] + 50), cv2.FONT_HERSHEY_PLAIN, 6, (255, 255, 255), 3)
            ...
            if distance < 50:
                for button in button_components:
                    if button.click(img, x, y) and delay_counter == 0:
                        ...
                        if len(equation) > 10:
                            equation_label.shrink(button.value, 6, img)

                        if button.value == "=":
                            equation = str(eval(equation))
                        elif (
                                button.value == "+" or button.value == "-" or button.value == "*" or button.value == "/") and \
                                (equation.endswith("+") or equation.endswith("-") or equation.endswith(
                                    "*") or equation.endswith("/")) and \
                                not (str(button.value).isdigit()):
